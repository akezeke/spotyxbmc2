...
{
  if (g_spotifyInterface->m_session)
  {
...
{
  if (getAddonSetting("enable") == "false")
  {
    disconnect();
    return false;
  }
  ...
  if (!m_session)
  {
    ...
    m_config.cache_location = getAddonSetting("cachePath");
    // The path of the directory to store the settings. This must be specified.
    // Please read the documentation on preferred values.
    m_config.settings_location = getAddonSetting("cachePath");
    ...
    //set high bitrate
    if (getAddonSetting("highBitrate") == "true")
    {
      CLog::Log( LOGERROR, "Spotifylog: using high bitrate");
      sp_session_preferred_bitrate(m_session, SP_BITRATE_320k);
    }else
      CLog::Log( LOGERROR, "Spotifylog: NOT using high bitrate");
...
bool SpotifyInterface::getDirectory(const CStdString &strPath, CFileItemList &items)
{
  ...
  if (strPath.Left(35) == "musicdb://spotify/command/settings/" || getAddonSetting("enable") == "false")
  {
    CGUIDialogAddonSettings settings;
    ADDON::AddonPtr addon;
    const CStdString pluginId = "plugin.music.spotyXBMC";

    if (ADDON::CAddonMgr::Get().GetAddon(pluginId, addon))
    {
      settings.ShowAndGetInput(addon, true);
      if (getAddonSetting("enable") == "true")
        connect(false);
      else
        disconnect();
    }
...
void SpotifyInterface::getMainMenuItems(CFileItemList &items)
{
  ...
  //settings
  share.strPath.Format("musicdb://spotify/command/settings/");
...
bool SpotifyInterface::search(CStdString searchstring)
{
  ...
  int maxTracks = atoi(getAddonSetting("searchNoTracks")) + 1;
  int maxArtists = atoi(getAddonSetting("searchNoArtists")) + 1;
  int maxAlbums = atoi(getAddonSetting("searchNoAlbums")) + 1;
  
  sp_search_create(m_session, searchstring, 0, maxTracks * 10, 0, maxAlbums * 10, 0, maxArtists * 10, &cb_searchComplete, NULL);
  CStdString message;
...
bool SpotifyInterface::getBrowseToplistArtists(CFileItemList &items)
{
  if (reconnect())
  {
    ...
    else
    {
      ...
      showProgressDialog(message);
      sp_toplistregion region = getAddonSetting("topListRegion") == "1" ? SP_TOPLIST_REGION_EVERYWHERE : (sp_toplistregion)sp_session_user_country(m_session);
...
{
  CStdString username = getAddonSetting("username");
 
  if (username.IsEmpty())
  {
    CGUIDialogKeyboard::ShowAndGetInput(username, "Spotify username",false,false);
  }
  return username;
}
...
{
  CStdString password = getAddonSetting("password");

  if (password.IsEmpty())
  {
    CStdString message;
    message.Format("Spotify password");
    CGUIDialogKeyboard::ShowAndGetInput(password, message,false,true);
  }
  return password;
}

CStdString SpotifyInterface::getAddonSetting(CStdString key){
  ADDON::AddonPtr addon;
  const CStdString pluginId = "plugin.music.spotyXBMC";
  CStdString value = "";
